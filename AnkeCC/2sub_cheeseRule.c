...
int ruleking(int first, int second, int col1, int row1, int col2, int row2) { //將
    ...
    } else if ((second == 5 || second == 12) && row1 == row2) {//王見王//吃掉將
        printf("123");
        int obstacle = 0;
        if (row1 == row2) { // 在同一行上
            printf("456");
...
int rulepow(int first, int second, int col1, int row1, int col2, int row2) { //炮
    ...
    if ((col1 == col2 && row1 != row2) || (row1 == row2 && col1 != col2)) {//直走
        if (col1 == col2) {
            ...
            if (obstacle == 0 && second == 0) { //中間沒有東西 且 到的地方是空的
                cheese[col2][row2] = first;
                cheese[col1][row1] = 0;
                return 0;
            }
            if (obstacle == 1 && ((player == 1 && second > 7) || (player == 2 && second < 8)) && second != 0) { //中間有一個東西 且 吃的合理
                cheese[col2][row2] = first;
                cheese[col1][row1] = 0;
                if (second != 0) {
                    status(5);//吃到東西了
